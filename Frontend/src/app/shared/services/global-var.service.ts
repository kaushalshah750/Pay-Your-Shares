...
export class GlobalVarService {
  ...
  async checkToken() {
    ...
    if (isValid == "Token Not Found") {
      this.snackBarService.openErrorSnackbar("You are not Authorized")
  ...
  getToken(code: string): Observable<AuthUser> {
    ...
    return this.http.post<AuthUser>(`${this.tokenUrl}?code=${code}&redirect_uri=${window.location.origin}/login&client_id=${environment.google.client_id}&client_secret=${environment.google.client_secret}&grant_type=authorization_code&response_type=token&scope=https://www.googleapis.com/auth/userinfo.email`, Header);
  }
