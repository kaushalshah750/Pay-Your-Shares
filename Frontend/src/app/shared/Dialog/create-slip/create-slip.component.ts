...
export class CreateSlipComponent {
  ...
  sendEmail(slip:AddSlip){
    slip.Split_between.forEach((member:number) => {
      ...
      var email:EmailBody = {
        User: member,
        Subject: "Split Generated by " + (this.globalVar.user.User_id == user.User_id ? "You" : this.globalVar.user.Name) + " of " + this.currency.transform(slip.Amount, this.homeCurrency) + " for " + slip.Name,
