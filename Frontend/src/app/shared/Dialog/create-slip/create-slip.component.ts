...
export class CreateSlipComponent {
    ...
    if(this.createform.valid){
      if(newUsers.length == 1 ? this.createform.controls['paidByUserId'].value != newUsers[0] : true){
        ...
        var Slip:AddSlip = {
          ...
          AddedBy_id: this.globalVar.user.User_id,
  ...
  sendEmail(slip:AddSlip){
    ...
    //   var slipOwner = this.data.members.filter(user => user.User_id == slip.paidUser_id)[0]
    //   var own = slip.amount / slip.split_between.length
    //   var email:EmailBody = {
    //     user: member,
    //     subject: "Split Generated by " + (this.globalVar.user.id == user.User_id ? "You" : this.globalVar.user.name) + " of " + this.currency.transform(slip.amount, this.homeCurrency) + " for " + slip.name,
    //     // body: ""
    ...
    //     <br>
    //     You Own <b>${this.currency.transform(own, this.homeCurrency)}</b> ${slip.paidUser_id == user.User_id ? "" : "to <b>"+ slipOwner.Name +"</b>"}
    //     ${slip.paidUser_id == user.User_id ? "" : "<br>Your Current Balance is <b>" + this.currency.transform(this.calculatesingledebt(slip.paidUser_id, user.User_id) - own, this.homeCurrency) + "</b>"}
    //     <br><br>
    //     <b>
    //     Best Regards
    //     <br>
